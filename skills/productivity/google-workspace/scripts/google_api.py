...
def gmail_send(args):
    ...
    if args.draft:
        result = service.users().drafts().create(userId="me", body={"message": body}).execute()
        print(json.dumps({"status": "draft_created", "id": result["id"]}, indent=2))
    else:
...
def sheets_clear(args):
    if _gws_binary():
        ...
        print(json.dumps({"clearedRange": result.get("clearedRange", "")}, indent=2))
        return
...
def main():
    ...
    send_parser = gmail_sub.add_parser("send")
    ...
    send_parser.add_argument("--from", dest="from_header")
    ...
    send_parser.add_argument("--attachments", help="Comma-separated file paths to attach")
    send_parser.set_defaults(func=gmail_send)
